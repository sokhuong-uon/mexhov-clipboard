...
pub fn get_caret_position() -> Option<(f64, f64)> {
    get_caret_via_gui_thread().or_else(get_caret_via_uia)
}

fn get_caret_via_gui_thread() -> Option<(f64, f64)> {
    unsafe {
        ...
        if ClientToScreen(caret_window, &mut screen_point) == 0 {
            return None;
        }
        Some((screen_point.x as f64, screen_point.y as f64))
    }
}

fn get_caret_via_uia() -> Option<(f64, f64)> {
